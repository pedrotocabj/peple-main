...
function cambiarPersona(data){
  ...
  const cambiarTabla =document.getElementById('container');
  cambiarTabla.innerHTML = `
  <div id="presentation">
  <p>${name} ${surname}</p>
  </div>
  <div id="image"><img src= ${picture}></div>
  ...
  container.append(cambiarTabla);
}

async function cambiarTexto() {
  fetch('https://randomuser.me/api/')
    .then((response) => response.json())
    .then((data) => cambiarPersona(data))
}
